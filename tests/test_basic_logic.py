...
class TestBasicClient(object):
    ...
    def test_cannot_receive_pushed_stream_when_enable_push_is_0(self, frame_factory):
        """
        If we have set SETTINGS_ENABLE_PUSH to 0, receiving PUSH_PROMISE frames
        triggers the connection to be closed.
        """
        c = h2.connection.H2Connection(initial_settings={
        ...
        expected_frame = frame_factory.build_goaway_frame(
            ...
            additional_data=b'PUSH_PROMISE: push disabled'
        )
    ...
    @pytest.mark.skip(reason="does not work well with nghttp2")
    def test_oversize_headers(self):
        ...
        c.local_settings[h2.settings.SettingCodes.MAX_FRAME_SIZE] = 16384
        c.initiate_connection()
    ...
    @pytest.mark.skip("does not work with nghttp2")
    def test_cannot_create_new_outbound_stream_over_limit(self, frame_factory):
        ...
        print(c.receive_data(f.serialize())[0])
    ...
    def test_reject_trailers_not_ending_stream(self, frame_factory):
        ...
        expected_frame = frame_factory.build_rst_stream_frame(
            stream_id=1, error_code=h2.errors.ErrorCodes.PROTOCOL_ERROR,
        )
        assert c.data_to_send() == expected_frame.serialize()

    @pytest.mark.skip("discrepancy between hpack and nghttp2")
    def test_can_send_trailers(self, frame_factory):
        ...
        print("Should send", f1.serialize())
        print("Should send", f2.serialize())
    ...
    def test_changing_max_frame_size(self, frame_factory, frame_size):
        ...
        print(c.receive_data(settings_ack.serialize()))
...
class TestBasicServer(object):
    ...
    def test_cannot_receive_new_streams_over_limit(self, frame_factory):
        ...
        assert b'request HEADERS: max concurrent streams exceeded' in c.data_to_send()
    ...
    def test_receiving_goaway_frame(self, frame_factory):
        ...
        f = frame_factory.build_goaway_frame(
            last_stream_id=0, error_code=h2.errors.ErrorCodes.SETTINGS_TIMEOUT
        )
        events = c.receive_data(f.serialize())
    ...
    def test_receiving_goaway_frame_with_unknown_error(self, frame_factory):
        ...
        f = frame_factory.build_goaway_frame(
            last_stream_id=0, error_code=0xFA
        )
        events = c.receive_data(f.serialize())
